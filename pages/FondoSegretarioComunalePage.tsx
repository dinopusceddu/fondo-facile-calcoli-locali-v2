...
interface FundingItemProps {
  id: keyof FondoSegretarioComunaleData;
  description: string;
  riferimentoNormativo: string;
  value?: number;
  isSubtractor?: boolean; 
  onChange: (field: keyof FondoSegretarioComunaleData, value?: number) => void;
    ...
    <div className={`grid grid-cols-12 gap-x-4 gap-y-2 py-4 border-b border-[#f3e7e8] last:border-b-0 items-start ${disabled ? 'opacity-60 bg-[#fcf8f8]' : ''}`}>
      ...
      <div className="col-span-12 md:col-span-4">
        <div className="relative">
            ...
            inputClassName={`text-right w-full h-11 p-2.5 ${isPercentage ? 'pr-8' : ''} ${disabled ? 'bg-[#e0d8d8] cursor-not-allowed' : 'bg-[#f3e7e8]'}`}
...
export const FondoSegretarioComunalePage: React.FC = () => {
  ...
  }, [data.fin_totaleRisorseRilevantiLimite, totaleRisorseRilevantiLimiteCalcolato, dispatch]); // Corrected dependency array
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <Card title="RISORSE STABILI" className="mb-6" isCollapsible defaultCollapsed={false}>
        ...
        <FundingItem id="st_art3c6_CCNL2011_retribuzionePosizione" description="A seguito del conglobamento di cui al comma 5, con decorrenza dal 31.12.2009, i valori complessivi annui lordi, per tredici mensilità, della retribuzione di posizione dei segretari comunali e provinciali, di cui all'art. 3 del CCNL del 16 maggio 2001 per il biennio economico 2000-2001, sono così determinati." riferimentoNormativo={RIF_CCNL_SEG_01032011_ART3C6} value={data.st_art3c6_CCNL2011_retribuzionePosizione} onChange={handleChange} />
        <FundingItem id="st_art58c1_CCNL2024_differenzialeAumento" description="Con decorrenza dal 1° gennaio 2021, i valori complessivi annui lordi, per tredici mensilità, della retribuzione di posizione dei segretari comunali e provinciali, di cui all’art. 107, comma 1 del CCNL del 17.12.2020 sono rideterminati come indicato nella seguente tabella (riportare il solo differenziale di aumento rispetto il CCNL precedente che non rileva ai fini del limite)" riferimentoNormativo={RIF_CCNL_SEG_16072024_ART58C1} value={data.st_art58c1_CCNL2024_differenzialeAumento} onChange={handleChange} />
        <FundingItem id="st_art60c1_CCNL2024_retribuzionePosizioneClassi" description="La retribuzione di posizione è erogata, in base alle classi demografiche degli enti, entro i seguenti valori minimi e massimi complessivi annui lordi per tredici mensilità." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART60C1} value={data.st_art60c1_CCNL2024_retribuzionePosizioneClassi} onChange={handleChange} />
        <FundingItem id="st_art60c3_CCNL2024_maggiorazioneComplessita" description="Nei comuni capoluogo, nelle province e nelle città metropolitane la soglia massima della retribuzione di posizione di cui al comma 1 può essere autonomamente rideterminata, per tener conto dell’esercizio delle funzioni in presenza di strutture complesse, in misura non superiore al 15%, ove sussista la relativa capacità di bilancio e nel rispetto dell’art. 23, comma 2 del D. Lgs. n. 75/2017." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART60C3} value={data.st_art60c3_CCNL2024_maggiorazioneComplessita} onChange={handleChange} />
        <FundingItem id="st_art60c5_CCNL2024_allineamentoDirigEQ" description="Gli enti assicurano, altresì, che nel complessivo rispetto dell’art. 23 comma 2 del D. Lgs. 75/2017 la retribuzione di posizione non sia inferiore a quella stabilita nell’Ente per l’incarico dirigenziale più elevato in essere o, in assenza di dirigenti, a quella più elevata, stabilita nell’Ente, per il personale con incarico di elevata qualificazione." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART60C5} value={data.st_art60c5_CCNL2024_allineamentoDirigEQ} onChange={handleChange} />
        <FundingItem id="st_art56c1g_CCNL2024_retribuzioneAggiuntivaConvenzioni" description="La struttura della retribuzione dei segretari comunali e provinciali si compone delle seguenti voci: g) retribuzione aggiuntiva per sedi convenzionate, ove spettante (risorse che non rilevano sul limite del salario accessorio)." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART56C1G} value={data.st_art56c1g_CCNL2024_retribuzioneAggiuntivaConvenzioni} onChange={handleChange} />
        <FundingItem id="st_art56c1h_CCNL2024_indennitaReggenzaSupplenza" description="La struttura della retribuzione dei segretari comunali e provinciali si compone delle seguenti voci: h) indennità di reggenza o supplenza ove spettante, per gli incarichi di cui all’art. 62 (risorse che non rilevano sul limite del salario accessorio)." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART56C1H} value={data.st_art56c1h_CCNL2024_indennitaReggenzaSupplenza} onChange={handleChange} />
      ...
      <Card title="RISORSE VARIABILI" className="mb-6" isCollapsible defaultCollapsed={false}>
        {/* ... FundingItem components ... */}
        <FundingItem id="va_art56c1f_CCNL2024_dirittiSegreteria" description="La struttura della retribuzione dei segretari comunali e provinciali si compone delle seguenti voci: f) diritti di segreteria, ove spettanti in base alle vigenti disposizioni di legge in materia (risorse che non rilevano sul limite del salario accessorio)." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART56C1F} value={data.va_art56c1f_CCNL2024_dirittiSegreteria} onChange={handleChange} />
        <FundingItem id="va_art56c1i_CCNL2024_altriCompensiLegge" description="La struttura della retribuzione dei segretari comunali e provinciali si compone delle seguenti voci: i) altri compensi previsti da norme di legge (risorse che non rilevano sul limite del salario accessorio)." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART56C1I} value={data.va_art56c1i_CCNL2024_altriCompensiLegge} onChange={handleChange} />
        <FundingItem id="va_art8c3_DL13_2023_incrementoPNRR" description="Per i segretari comunali e provinciali, la medesima facoltà di incremento percentuale (in misura non superiore al 5 per cento) del trattamento accessorio oltre il limite di cui all'articolo 23, comma 2, del decreto legislativo 25 maggio 2017, n. 75, è calcolata sui valori della retribuzione di posizione, spettanti in base all'ente di titolarità, come definiti dal comma 1 dell'articolo 107 del contratto collettivo nazionale di lavoro relativo al personale dell'area delle funzioni locali, sottoscritto in data 17 dicembre 2020, nonché sul valore della retribuzione di risultato come risultante dai contratti collettivi vigenti." riferimentoNormativo={RIF_ART8_DL13_2023} value={data.va_art8c3_DL13_2023_incrementoPNRR} onChange={handleChange} />
        <FundingItem id="va_art61c2_CCNL2024_retribuzioneRisultato10" description="Gli enti destinano a tale compenso (retribuzione di risultato), con risorse a carico dei rispettivi bilanci e nei limiti della propria capacità di spesa e nel rispetto dell’art. 23, comma 2 del D. Lgs. n. 75/2017, un importo non superiore al 10% del monte salari erogato a ciascun segretario nell’anno a cui è riferita la valutazione ai sensi del comma 1." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART61C2} value={data.va_art61c2_CCNL2024_retribuzioneRisultato10} onChange={handleChange} />
        <FundingItem id="va_art61c2bis_CCNL2024_retribuzioneRisultato15" description="Gli enti possono elevare fino al 15% il limite percentuale di cui al comma 2, fermi restando i limiti della propria capacità di spesa ed il rispetto dell’art. 23, comma 2 del D. Lgs. n. 75/2017, nei casi di seguito indicati, limitatamente al periodo di svolgimento delle relative funzioni: a) segretari di enti con dirigenza; b) segretari di enti privi di dirigenza a cui sia stato attribuito un incarico per la copertura di posizione apicale dell’ente temporaneamente priva di titolare, formalmente affidato in conformità all’ordinamento di ciascun ente; c) segretari a cui siano attribuite le funzioni di segretario di una Unione di comuni; d) enti interessati da situazioni di calamità naturale." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART61C2BIS} value={data.va_art61c2bis_CCNL2024_retribuzioneRisultato15} onChange={handleChange} />
        <FundingItem id="va_art61c2ter_CCNL2024_superamentoLimiteMetropolitane" description="I limiti di cui ai commi 2 e 2-bis possono essere superati negli enti metropolitani, fermi restando i limiti della propria capacità di spesa e nel rispetto dell’art. 23, comma 2 del D. Lgs. n. 75/2017, qualora sia valutata l’esigenza di un allineamento rispetto alle retribuzioni complessive di livello più elevato corrisposte alla dirigenza dell’ente. A tal fine si dovrà tenere conto di tutte le componenti retributive corrisposte al Segretario, ivi compreso il cosiddetto galleggiamento e, comunque, non potrà consentire il superamento delle predette retribuzioni dirigenziali." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART61C2TER} value={data.va_art61c2ter_CCNL2024_superamentoLimiteMetropolitane} onChange={handleChange} />
        <FundingItem id="va_art61c3_CCNL2024_incremento022MonteSalari2018" description="In attuazione di quanto previsto dall’art. 1, comma 604 della L. n. 234/2021 (Legge di bilancio 2022), con la decorrenza ivi indicata, le amministrazioni possono incrementare, in base alla propria capacità di bilancio, le risorse di cui al comma 2 (retribuzione di risultato), di un importo non superiore allo 0,22% del monte salari 2018 relativo ai segretari comunali e provinciali. Tali risorse, in quanto finalizzate a quanto previsto dall'articolo 3, comma 2, del D.L. n. 80/2021, non sono sottoposte al limite di cui all’art. 23, comma 2 del D. Lgs. n. 75/2017." riferimentoNormativo={RIF_CCNL_SEG_16072024_ART61C3} value={data.va_art61c3_CCNL2024_incremento022MonteSalari2018} onChange={handleChange} />
        <SectionTotal label="SOMMA RISORSE VARIABILI" total={sommaRisorseVariabili} />
      </Card>
      
      <Card title="RIEPILOGO E ADEGUAMENTI FINALI" className="mb-6" isCollapsible defaultCollapsed={false}>
        <FundingItem id="fin_totaleRisorseRilevantiLimite" description="Totale risorse che rilevano ai fini del limite del salario accessorio" riferimentoNormativo="Calcolato automaticamente sulla base delle voci rilevanti e della % di copertura." value={data.fin_totaleRisorseRilevantiLimite} onChange={handleChange} disabled={true} inputInfo={infoTotaleRisorseRilevantiLimite}/>
        <SectionTotal label="TOTALE RISORSE (Stabili + Variabili)" total={totaleRisorse} />
        <FundingItem id="fin_percentualeCoperturaPostoSegretario" description="Percentuale di copertura del posto di Segretario a carico dell'Ente nell'anno di riferimento" riferimentoNormativo="Es. 100% se interamente a carico, 50% se condiviso con altro ente, ecc." value={data.fin_percentualeCoperturaPostoSegretario} onChange={handleChange} isPercentage={true}/>
      </Card>

      <Card title="TOTALE RISORSE EFFETTIVAMENTE DISPONIBILI PER SEGRETARIO COMUNALE" className="mb-6 bg-[#f3e7e8] border-[#d1c0c1]">
        ...
        <p className="text-xs text-center text-[#1b0e0e] mt-2">
            Calcolato come: TOTALE RISORSE * (Percentuale Copertura / 100).
        </p>
      </Card>
    </div>
  );
