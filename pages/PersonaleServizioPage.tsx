...
import { TEXTS_UI, ALL_LIVELLI_PEO, ALL_AREE_QUALIFICA, ALL_TIPI_MAGGIORAZIONE, PROGRESSION_ECONOMIC_VALUES, INDENNITA_COMPARTO_VALUES } from '../constants.js';
...
export const PersonaleServizioPage: React.FC = () => {
  ...
  const totalAbsorbedIndennitaComparto = React.useMemo(() => {
    return (personaleServizioDettagli || []).reduce((sum, employee) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <Card title="Riepilogo Indennità di Comparto Assorbita" className="mt-8">
        <div className="p-4 text-center">
          <p className="text-sm text-[#5f5252] mb-1">Importo Totale Indennità di Comparto Assorbita dal Fondo</p>
          <p className="text-2xl font-bold text-[#ea2832]">
            {formatCurrency(totalAbsorbedIndennitaComparto)}
          </p>
          <p className="text-xs text-[#5f5252] mt-2">
            Il calcolo considera la percentuale di part-time e i periodi di servizio effettivo nell'anno di riferimento.
          </p>
      ...
      <Card title="Totale Risorse Assorbite dal Fondo" className="mt-8 bg-[#f3e7e8] border-[#d1c0c1]">
        <div className="p-4 text-center">
          <p className="text-sm text-[#5f5252] mb-1">Somma Totale delle Progressioni Economiche e Indennità di Comparto</p>
          <p className="text-2xl font-bold text-[#ea2832]">
            {formatCurrency(totalAbsorbed)}
          </p>
        </div>
      </Card>
